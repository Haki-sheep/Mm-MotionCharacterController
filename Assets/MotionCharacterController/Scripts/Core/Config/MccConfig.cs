...
namespace MotionCharacterController
{
    ...
    public partial class MccConfig
    {
        ...
        [Header("地面")]
        [Tooltip("旧参数保留项：最大可站立坡度。新逻辑主要使用 Max Stable Slope Angle。")]
        public float maxSlopeAngle = 60f;
        [Tooltip("角色能稳定站住的最大坡度角。比如 60 表示小于等于 60 度的斜坡都算地面。")]
        public float maxStableSlopeAngle = 60f;
        [Tooltip("向脚下探测地面的基础距离。太小可能下坡时容易短暂离地，太大可能吸地感过强。")]
        public float groundProbeDistance = 0.2f;
        [Tooltip("额外增加的地面探测距离。高速移动、下坡、台阶场景可以适当加大。")]
        public float groundDetectionExtraDistance = 0f;
        [Tooltip("哪些层可以被当作稳定地面检测。通常保持 Everything，特殊场景可只勾选 Ground。")]
        public LayerMask stableGroundLayers = -1;
        [Tooltip("是否开启离散碰撞事件。开启后，角色最终位置与碰撞体重叠时会回调 OnDiscreteCollisionDetected。")]
        public bool discreteCollisionEvents = false;

        [Header("台阶")]
        [Tooltip("台阶处理方式。None 不爬台阶；Standard 普通台阶；Extra 会额外检测更浅的小台阶。")]
        public StepHandlingMethod stepHandling = StepHandlingMethod.Standard;
        [Tooltip("角色能自动走上去的最大台阶高度。比如 0.5 表示半米以内的小台阶可以走上去。")]
        public float maxStepHeight = 0.5f;
        [Tooltip("没有稳定接地时是否仍允许上台阶。一般关闭，避免空中蹭墙时被台阶吸上去。")]
        public bool allowSteppingWithoutStableGrounding = false;
        [Tooltip("Extra 台阶模式下，台阶至少要有多深才允许踩上去。值越大越不容易爬很薄的边。")]
        public float minRequiredStepDepth = 0.1f;

        [Header("边缘")]
        [Tooltip("是否开启边缘和落差检测。开启后，角色站在平台边缘时会更稳定，不容易半个身子悬空还被当作站稳。")]
        public bool ledgeAndDenivelationHandling = true;
        [Tooltip("角色离边缘多远以内还算能稳定站住。通常不要大于胶囊半径。")]
        public float maxStableDistanceFromLedge = 0.5f;
        [Tooltip("朝平台外侧移动速度超过这个值时，阻止继续吸附地面。0 表示只要朝外移动就更容易离开边缘。")]
        public float maxVelocityForLedgeSnap = 0f;
        [Range(1f, 180f)]
        [Tooltip("地面内外法线落差超过这个角度时，不再认为角色能稳定贴地。用于防止陡坎处强行吸地。")]
        public float maxStableDenivelationAngle = 180f;

        [Header("刚体交互")]
        [Tooltip("是否处理移动平台、动态刚体推挤、站在刚体上跟随移动等逻辑。")]
        public bool interactiveRigidbodyHandling = true;
        [Tooltip("角色和动态刚体的交互方式。Kinematic 更像霸体角色；SimulatedDynamic 会按模拟质量推刚体。")]
        public RigidbodyInteractionType rigidbodyInteractionType = RigidbodyInteractionType.Kinematic;
        [Tooltip("模拟动态交互时角色的质量。质量越大，推箱子时越有力量。")]
        public float simulatedCharacterMass = 1f;
        [Tooltip("离开移动平台时是否保留平台速度。开启后从移动平台跳下会带一点惯性。")]
        public bool preserveAttachedRigidbodyMomentum = true;

        [Header("约束")]
        [Tooltip("是否把角色移动限制在某个平面上。普通 3D 角色一般关闭。")]
        public bool hasPlanarConstraint = false;
        [Tooltip("开启平面约束时使用的法线方向。比如 forward 表示限制在与 Z 轴垂直的平面上。")]
        public Vector3 planarConstraintAxis = Vector3.forward;

        [Header("求解安全")]
        [Tooltip("一次移动最多做多少次碰撞扫掠。越大越不容易卡墙角，但性能消耗更高。")]
        public int maxMovementIterations = 5;
        [Tooltip("角色一开始卡进墙里时，最多尝试推出几次。一般 1 到 3 就够。")]
        public int maxDecollisionIterations = 1;
        [Tooltip("移动前是否先检查初始重叠。开启后更不容易穿模，但会多一点检测开销。")]
        ...
        [Tooltip("超过最大移动迭代次数时是否丢弃剩余位移。开启能避免角色被挤进墙里。")]
        public bool killRemainingMovementWhenExceedMaxMovementIterations = true;

        [Header("系统")]
        [Tooltip("是否由 MCC 系统自动在 FixedUpdate 中模拟。普通用法保持开启。")]
        public bool autoSimulation = true;
        [Tooltip("是否开启角色显示位置插值。开启后画面更顺；如果相机或角色抖动，可临时关闭对比测试。")]
